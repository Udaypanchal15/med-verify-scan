...
export const UserMedicineDatabase = () => {
  ...
  // Load all approved medicines on mount
  useEffect(() => {
    loadAllMedicines();
  }, []);
  ...
  const loadAllMedicines = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/medicine/all`, {
      ...
      if (response.ok) {
        ...
        const categories = [...new Set(allMedicines.map(m => m.category).filter(Boolean))];
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/medicine/search?q=${encodeURIComponent(searchQuery)}`,
        {
          headers: getAuthHeader(),
        }
      );
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: `Found ${searchResults.length} medicine(s)`,
        });
      } else {
        toast({
          title: "Search Failed",
          description: "Could not search medicines",
          variant: "destructive",
        });
      }
    } catch (error) {
    ...
    <section id="database" className="py-20 bg-secondary/20">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold mb-4">Browse Medicines</h2>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Search and browse all approved medicines available from verified sellers.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    placeholder="Search by medicine name, category, or seller..."
              ...
              <div className="flex gap-2 flex-wrap">
                {medicineCategories.length > 0 ? (
                  medicineCategories.slice(0, 5).map((category) => (
                    <Button
                      key={category}
                      variant="outline"
                      size="sm"
        ...
        {!loading && effectiveResults.length > 0 && (
          <div className="max-w-7xl mx-auto">
            <div className="mb-6 flex justify-between items-center">
              <div>
                <h3 className="text-2xl font-semibold mb-2">Available Medicines</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-lg">
                      ...
                      <span className="font-medium text-sm truncate">
                        {medicine.company_name || 'Unknown Seller'}
                      </span>
                    </div>
          ...
          !selectedCategory && (
            <Card className="max-w-2xl mx-auto p-12 text-center">
              ...
              <p className="text-muted-foreground mb-6">
                Approved medicines from verified sellers will appear here.
              </p>
            </Card>
          )}
      </div>
